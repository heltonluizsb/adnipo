<?php 
	...
	if(isset($_POST['acao']) && $_POST['acao'] == 'carrega_novos_posts'){
		...
		if($sql->rowCount() > 0){
			...
			$data['sucesso'] = true;
			$data['mensagem'] = 'existe dados a serem exibidos';
			...
			$data['sucesso'] = false;
			$data['mensagem'] = 'chegou no fim da pagina';
		}
	} else if(isset($_POST['acao']) && $_POST['acao'] == 'carrega_novas_respostas'){
		$por_pagina = $_POST['por_pagina'];
		$pagina_proxima = $_POST['pagina_proxima'];
		$post_id = $_POST['post_id'];
		$data['post_id'] = $post_id;
		$estalogado = $_POST['estalogado'];
		$user_id = $_POST['user_id'];
		$fim_respostas = $_POST['fim_respostas'];
		$data['fim_respostas'] = $fim_respostas;

		$usuario = MySql::conectar()->prepare("SELECT * FROM `tb_user` WHERE `id` = ?");
		$usuario->execute(array($user_id));
		$usuario = $usuario->fetch();

		$sql = MySql::conectar()->prepare("SELECT * FROM `tb_post` WHERE `resposta_id` = ? ORDER BY `id` DESC LIMIT $pagina_proxima,$por_pagina");
